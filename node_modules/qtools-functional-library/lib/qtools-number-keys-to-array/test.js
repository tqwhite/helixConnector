...
const moduleFunction = function(args) {
	...
	const completeStepActual = ({
		...
		if (verbose) {
			console.log(
				`Result for: ${methodName} (${thisStepPass ? 'PASS' : 'FAIL'}):`
			);
	...
	thisStepEvalFunction = item =>
		(item =
			item.length == 3 &&
			item.reduce(
				(item, result) => typeof (item != 'undefined') && result,
				true
			));
	passingTests =
		completeStep(
	...
	methodName = 'qtNumberKeysToArray';
	thisStepMessage =
		'test #3, result is wrong, non-numeric element is missing or length is wrong';
	thisStepResult = testArray[methodName]({
		collapseHoles: true,
		omitNonNumeric: false
	});
	thisStepEvalFunction = item =>
		(item = item.length == 4 && typeof item[3] == 'object');
	passingTests =
		completeStep(
			methodName,
			thisStepMessage,
	...
	thisStepResult = testArray2[methodName]({
		collapseHoles: true,
		omitNonNumeric: false
	});
	thisStepEvalFunction = item =>
		(item =
			item.length == 5 &&
			typeof item[4] == 'object' &&
			item[4].value.length == 3);
	passingTests =
		completeStep(
			methodName,
			thisStepMessage,
	...
	thisStepResult = emptyArray[methodName]({
		collapseHoles: true,
		omitNonNumeric: false
	});
