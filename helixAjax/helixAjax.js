...
var moduleFunction = function(args) {
	...
	var retrieveRecords = function(helixConnector, schema, criterion, callback) {
		...
		// 		const tmpParms=qtools.clone(retrievalParms);
		// 		tmpParms.callback=(err, result)=>{
		//
		// 	console.dir({"err [ helixConnector.process('retrieveRecords', retrievalParms);]":err});
		// 	console.dir({"result [ helixConnector.process('retrieveRecords', retrievalParms);]":result});
		// 	callback(err, result);
		// }
		// 		helixConnector.process('getViewDetails', tmpParms);
	...
	router.get(/hxConnectorCheck/, function(req, res, next) {
		res.status('200');
		res.send(
			"<div style='font-size:24pt;padding:150px;'>hxConnector status: <span style='color:green;'>active</span><br/>helix server status: <span style='color:gray;'>[check disabled]</span></div>"
		);
	});
