...
var moduleFunction = function(args) {
	...
	self.stringToBoolean = function(item) {
		...
		switch (item.toLowerCase()) {
			...
			case 'no':
				...
				break;
			default:
				qtools.log(`WARNING: no such boolean mapping for ${item}`);
				throw new Error(`no such boolean mapping for ${item}`);
				break;
		}
	};
			...
			case 'toJson':
				...
				switch (value.toString().toLowerCase()) {
					...
					case 'no':
						return false;
					default:
						qtools.log(`WARNING: ${value} is not a valid boolean value`);
						throw new Error(`no such boolean mapping for `+value);
						break;
				}
				break;
		}
	...
	self.DateTimeType = (value = '', destination) => {
		...
		if (result == 'Invalid Date') {
			throw new Error(
				`String '${value}' cannot be parsed into date (helixData.DateTimeType)`
			);
		}
		switch (destination) {
			case 'toHelix':
	...
	self.NumberType = (value, destination) => {
		...
		if (typeof +value != 'number') {
			throw new Error(
				`String '${value}' cannot be parsed into a number (helixData.NumberType)`
			);
		}
...
console.log(`\n=-=============   resultData  ========================= [helixData.js.self.helixStringToRecordList]\n`);


console.log("resultData="+resultData+" [helixData.js.self.helixStringToRecordList]");
...
console.log(`\n=-=============   resultData  ========================= [helixData.js.self.helixStringToRecordList]\n`);
		...
		for (var i = 0, len = resultDataArray.length; i < len; i++) {
			...
			for (var j = 0, len2 = inSchema.length; j < len2; j++) {
				...
				newElementObject[fieldName] = mappedData;
			}
