...
var moduleFunction = function(args) {
	...
	var helixBoolean = function(item) {
		...
		switch (item) {
			...
			case 'No':
				...
				break;
			default:
			throw (new Error("no such boolean mapping for " + item + ", says " + qtools.employerFilename));
				break;
		}
	...
	self.helixStringToRecordList = function(schema, resultData) {
		...
		resultData = resultData.replace(new RegExp(`${recordSeparator}+$`, 'g'), '').replace(/record id:(\d+), /g, '$1'+fieldSeparator).replace(/helix record:/g, '');
		...
		for (var i = 0, len = resultDataArray.length; i < len; i++) {
			var elementList = resultDataArray[i].replace(new RegExp(`${fieldSeparator}$`), '').split(new RegExp(`${fieldSeparator}`));
