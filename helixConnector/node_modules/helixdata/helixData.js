...
var moduleFunction = function(args) {
	...
	self.stringToBoolean = function(item) {
		...
		switch (item.toLowerCase()) {
			...
			default:
				qtools.log(
					`WARNING: no such boolean mapping for ${item}, says ${stringToBoolean}/${stringToBoolean}`
				);
				throw new Error(
					`no such boolean mapping for ${item}, says ${stringToBoolean}/${stringToBoolean}`
				);
				break;
		}
	};
		...
		for (let i = 0, len = data.length; i < len; i++) {
			...
			for (let j = 0, len2 = dataArray.length; j < len2; j++) {
				...
				recordObject[elementName] = mapping[elementName]
					? self[mapping[elementName]](dataArray[j])
					: dataArray[j];
			}
	...
	self.remoteControlConversionList.cleanToJson = (
		...
		if (!result){
			callback('', {error: `data source provide no result (${typeof(result)}).`});
			return;
		}

		let outData;
		const cleanString=result.replace(/^\"/, "").replace(/(]|})[^\}\]]*$/, "$1").replace(/\\/g, "");
		try {
