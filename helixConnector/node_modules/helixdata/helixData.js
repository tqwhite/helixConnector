...
var moduleFunction = function(args) {
	...
	self.DateTimeType = (value, destination) => {
		...
		switch (destination) {
			case 'toHelix':
				//helix example: '6/29/15  8:38:39 AM'
				var options = {year: 'numeric', month: 'numeric', day: 'numeric', hour:'numeric',  minute:'numeric', hour12:true, hourCycle:'h12', second:'numeric'};
				const newDate=new Date(result).toLocaleString('en-US', options).replace(/,/g, '');
				return newDate;
				break;
			case 'toJson':
	...
	const {fieldSequenceList, mapping}=schema;
		...
		if (qtools.isTrue(schema.debugData) && !schema.internalSchema){
				const filePath=`${process.env.HOME}/Desktop/tmp/file_FromJson_${new Date().getTime()}_${schema.schemaName}.txt`;
	...
	self.stringifyObject = function(
		...
		for (var i = 0, len = schema.length; i < len; i++) {
			var element = schema[i],
				mappingEntry = mapping[element]?mapping[element]:'StringType',
				finalFunction;
